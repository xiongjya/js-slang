...
const heap_get_child = (address: any, child_index: any) =>
...
const heap_get_size = (address: any) =>
...
const heap_get_string_hash = (address: any) =>
	heap_get_4_bytes_at_offset(address, 1);

const heap_get_string = (address: any) =>
...
const heap_get_Callframe_pc = (address: any) =>
...
const builtin_object = {
	...
	math_sqrt: () =>
		JS_value_to_address(Math.sqrt(address_to_JS_value(OS.pop()))),
...
for (let i = 0; i < primitive_values.length; i++) {
	...
	if (
		typeof primitive_value === "object" && primitive_value.hasOwnProperty("id")
	) {
